...
pub async fn parse_stun_packet(packet: &[u8]) -> Option<StunBindingRequest> {
    if packet.len() < STUN_HEADER_LEN {
        return None;
    }
    let mut reader = BufReader::new(packet);
    let message_type = reader.read_u16().await.ok()?;
    if message_type != StunType::BindingRequest as u16 {
        return None;
    }

    let length = reader.read_u16().await.ok()?;
    if length % 4 != 0 || STUN_HEADER_LEN + length as usize > packet.len() {
        return None;
    }

    let magic_cookie = reader.read_u32().await.ok()?;
    if magic_cookie != STUN_COOKIE {
        return None;
    }

    let mut transaction_id = [0; 12];
    reader.read(&mut transaction_id).await.ok()?;


    let mut attributes: Vec<StunAttribute> = Vec::new();

    while let Ok(attribute_type_key) = reader.read_u16().await {
        ...
        let mut length = reader.read_u16().await.unwrap();
        length = pad_to_4bytes(length);
        let mut value_buffer: Vec<u8> = vec![0; length as usize];
        reader.read_exact(&mut value_buffer).await.unwrap();
